...
from vega_tools.common.pandas_tools import read_structured_file, write_structured_file, audit_images, \
    ...
    split_csv_to_txt, repackage_txts_to_csv
...
SPARK_NLP_DIR = Path(__file__).resolve().parent.parent.parent / 'spark-nlp'
PHILTER_UCSF_DIR = Path(__file__).resolve().parent.parent.parent / 'philter' / 'philter-ucsf'
...
@cli.command()
@click.option('--sample', '-s', type=click.Path(exists=True), required=True,
...
@click.option(
    '--python', 'python_executable', envvar='PHILTER_PYTHON', default='python', show_default=True,
...
def philter(sample, result, python_executable):
    ...
    try:
        ...
        cmd = [
            python_executable, str(PHILTER_UCSF_DIR / 'main.py'),
            '-i', str(input_path),
            '-o', str(output_path),
